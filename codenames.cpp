...
struct SimilarityEngine{
	...
	const vector<double>& getVec(const string& s) {
		return vec.at(s);
	}
...
struct Bot{
	...
	pair<double, int> getWordScore(const string& word, bool debugPrint) {
		if(debugPrint)
		...
		rep(i,0,boardWords.size()){
			if(boardWords[i].word.find(word) != string::npos)
				return make_pair(-1000, -1);
			if(word.find(boardWords[i].word) != string::npos)
				return make_pair(-1000, -1);
		}

		const vector<double>& wordVec = engine.getVec(word);

		typedef pair<double, BoardWord*> Pa;
		static vector<Pa> v;
		v.clear();
		rep(i,0,boardWords.size()){
			double sim=engine.similarity(boardWords[i].vec, wordVec);
		...
		rep(i,0,v.size()){
			...
			if (type == 'o') weight = fuzzyWeightOpponent;
			if (type == 'm') weight = fuzzyWeightMy;
			if (type == 'g') weight = fuzzyWeightGrey;
	...
	pair<string, int> getBestWord(
		...
		createBoardWords();
		vector<pair<double, pair<int, const string*>>> v;
		...
		for(const string* candidate : candidates){
			pair<double, int> res=getWordScore(*candidate, false);
		...
		rep(i,0,20){
			pair<double, int> res = getWordScore(*v[i].second.second, false);
